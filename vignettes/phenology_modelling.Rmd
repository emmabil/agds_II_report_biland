...
# Libraries
library(phenocamr);library(terra);library(dplyr);library(here);library(ggplot2);library(GenSA);library(MODISTools);library(geosphere)
```
<br>
### **Introduction**
...
predicted_phenology_tri <- terra::app(
ma_nh_temp,
...
modis_ma_nh <- MODISTools::mt_subset(
product   = "MCD12Q2",
...
modis_ma_nh_rast <- MODISTools::mt_to_terra(
df        = modis_ma_nh,
...
modis_harvard <- MODISTools::mt_subset(
  product   = "MCD12Q2",
  band      = "Greenup.Num_Modes_01",
  lat       = harvard_lat,
  lon       = harvard_lon,
...
modis_harvard_ts <- modis_harvard |>
  ...
  dplyr::filter(!is.na(greenup_doy))
