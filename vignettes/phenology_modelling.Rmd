...
# garder seulement les années pour lesquelles on a une observation
data_start$drivers <- subset(
  data_start$drivers,
  year %in% data_start$validation$year
)

# par = c(temp_threshold, gdd_crit, doy_start)
par0_start   <- c(0, 150, 1)         # départ : commence le 1er janvier
lower_start  <- c(-10, 50, 1)        # seuils plausibles
upper_start  <- c(10, 400, 120)      # DOY_start max = 120 (fin avril)
