...
### Optimisation du modèle GDD + photopériode

data_photo <- list(
  drivers    = harvard_temp,
  validation = harvard_phenology
)
#there is missing value for 2023, so I will skipped this year
data_photo$drivers <- subset(
  data_photo$drivers,
  year %in% data_photo$validation$year)
  
# par = c(temp_threshold, gdd_crit, photoperiod_crit)
par0_photo   <- c(0,   200,  11)   # point de départ
lower_photo  <- c(-5,  50, 12)
upper_photo  <- c(10, 400, 16)
...
optim_photo <- optim(
  ...
  lower   = lower_photo,
  upper   = upper_photo,
  control = list(maxit = 300)   # 300 itérations suffisent largement
)

optim_par_photo <- optim_photo$par
optim_par_photo


```
```{r prediction}
# join predicted with observed data (photo-thermal model)
validation_photo <- left_join(predictions_photo, harvard_phenology)
...
ggplot(validation_photo) +
  ...
  labs(
    x = "Observed leaf-out date (DOY)",
    y = "Predicted leaf-out date (DOY, GDD + photoperiod)"
  ) +
  theme_bw() +
  theme(
    legend.position = "none"
